...
usr_question = usr_question.strip().split()
...
with open('question2vec_pair.txt', encoding='utf-8') as f:
...
for key in question2vec_dic:
    cosimilar_dic[key] = cosine_similarity(question2vec_dic[key], usr_question2vec[usr_question[0]])

# 对‘问题句子：相似性’字典按value值排序，获取相似性高的前十个句子
sorted_questions = sorted(cosimilar_dic.items(), key=lambda kv:(kv[1], kv[0]), reverse=True)
