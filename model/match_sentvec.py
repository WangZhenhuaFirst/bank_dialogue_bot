...
kmeans = load('data/kmeans_1000.joblib')
tfidf_vectorizer = load('data/tfidf_vectorizer_1000.joblib')
...
def sentvec_match(user_input):
    ...
    user_input = [" ".join(x for x in jieba.cut(user_input) if x not in stopwords)]
    category = kmeans.predict(tfidf_vectorizer.transform(user_input))[0]
    print(f"类别: {category}")
    ...
    print(f"sorted_questions: {sorted_questions}")
